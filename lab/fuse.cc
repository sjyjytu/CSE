                        ...
                        off_t off, struct fuse_file_info *fi)
{
    ...
    std::string entries;
    yfs->readdir(inum, entries);
    // entries example:  a.c/123/b.o/145/c.d/156/
    while (entries.size() > 1)
    {
        int cut_pos = entries.find_first_of('/');
        if (cut_pos == -1)
            break;
        std::string file_name = entries.substr(0, cut_pos);
        entries = entries.substr(cut_pos + 1);
        cut_pos = entries.find_first_of('/');
        yfs_client::inum file_inum = yfs_client::n2i(entries.substr(0, cut_pos));
        dirbuf_add(&b, file_name.c_str(), (fuse_ino_t)file_inum);
        if (cut_pos == -1)
        {
            fuse_reply_err(req, ENOENT);
            return;
        }
        if ((unsigned int)cut_pos == entries.size() - 1)
            break;
        entries = entries.substr(cut_pos + 1);
    }
